...
%}
\captionof{figure}{ 95\% $CL_{S}$ CL on the lightest neutralino production cross-section times branching ratio~($\sigma_{\mbox{\tiny{sig}}}\times BR_{\PSneutralinoOne}$)[\pba] against mean lifetime[\ns] for $\Lambda = 180$\TeV in the SPS8 benchmark GMSB model.}
...
\captionof{figure}{95\% $CL_{S}$ CL on the lightest neutralino production cross-section times branching ratio~($\sigma_{\mbox{\tiny{sig}}}\times BR_{\PSneutralinoOne}$) for different $\Lambda$~(or mass of lightest neutralino) for $\tau = 6.7$\ns~($c\tau = 2000\mm$) in the SPS8 benchmark GMSB model.}
...
\captionof{figure}{Exclusion limit in lightest neutralino mass~(or $\Lambda$) against mean lifetime in SPS8 benchmark GMSB model. Limits from previous experiments also shown.}
