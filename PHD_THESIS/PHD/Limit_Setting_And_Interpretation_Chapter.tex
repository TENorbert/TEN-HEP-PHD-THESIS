...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Model-dependent Limits and Interpretation}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Our results from section \ref{RES} can also be interpreted by deriving cross-section upper limits for the production of signal events as predicted by a particular BSM physics model like the SPS8 benchmark GMSB model. The cross-section upper is set using a statistical limit setting procedure called the $CL_{S}$ method. The $CL_{S}$ method provides a framework to compute probabilities known as $CL_{S}$ or $p$-values from likelihood functions for a range of cross-sections~($\sigma$) predicted by the model. The value of the cross-section for which the $p$-value reaches a probability of $0.05$~($0.1$) is the upper limit on the cross-section for producing new physics at 95\%~(90\%) confidence level. The range of cross-section values less than this upper limit are said to be excluded at the same percentage  confidence level. In practice $p$-values are computed and the upper limit is set on some \textit{Parameter Of Interest}~(POI) like the signal-strength modifier, $\mu$, of the BSM physics model. The signal-strength modifier is directly related to the production cross-section.
\newline 
The exclusion limit on the cross-section as predicted by the BSM model for the production of a new particle is easily translated into exclusion limits on other parameters of the model such as the mass and lifetime of the new particle allowing us to make quantitative statements like the following: 
\begin{itemize}
...
\end{itemize}
The uncertainties of the experiment like the detector inefficiencies and luminosity measurements which can impact the exclusion limits but are not the parameters under investigation by the analysis are taken into account in the $CL_{S}$ method through the use of \textit{nuisance parameters}. Nuisance parameters usually denoted as $\mathbb{\theta}$ are not the POI and are introduced in the $CL_{S}$ method at the level of formulating the likelihood function to take into account the effects of these uncertainties in the computation of the exclusion limits.
...
The \textsc{HiggsCombine} tool gives us an upper limit on signal-strength parameter, $\mu^{\mbox{\tiny{UL}}}$, which is equivalent to an upper limit on the product of the cross-section and branching ratio decay channel of the neutralino to a photon and gravitino, $\sigma_{\mbox{\tiny{sig}}}\times BR_{\PSneutralinoOne}$, as predicted by the SPS8 benchmark GMSB model. Because $\sigma_{\mbox{\tiny{sig}}}\times BR_{\PSneutralinoOne}$ depends on $m_{\PSneutralinoOne}$ or equivalently $\Lambda$ which are parameters of the  SPS8 benchmark GMSB model, the theoretical prediction by the SPS8 benchmark GMSB model at Leading Order~(LO) interactions of  $\sigma_{\mbox{\tiny{sig}}}\times BR_{\PSneutralinoOne}$ for $m_{\PSneutralinoOne} = 255\GeVcc$ is 0.015\pba which is indicated by the blue line in Figure  \ref{fig:SPS8_Ctau_Ulimit} and in general decreases with increasing values of $m_{\PSneutralinoOne}$ also shown by the blue line in Figure Figure \ref{fig:MASS-limits}.
...
\newline
Prior to this search experiment, an upper limit of $0.02\pba$ was set at 95\% CL on $\sigma_{\mbox{\tiny{sig}}}\times BR_{\PSneutralinoOne}$ by CMS for a neutralino with mass of 215\GeVcc and $\tau_{\PSneutralinoOne} = 0.003\ns$~($c\tau_{\PSneutralinoOne} = 1\mm$) \cite{CMS}. Figure \ref{fig:SPS8_SIGMA-Ulimit} presents the excluded $\sigma_{\mbox{\tiny{sig}}}\times BR_{\PSneutralinoOne}$ for the mass of the neutralino ranging from 140\GeVcc to 300\GeVcc as interpreted from the results of our search experiment. For example we set an upper limit of 0.04\pba for $m_{\PSneutralinoOne} = 215\GeVcc$ and $\tau_{\PSneutralinoOne} \simeq 1.1\ns$. The excluded cross-section for neutralinos with different mass and lifetime can be also read from Figure \ref{fig:SPS8_SIGMA-Ulimit}.  Figure  \ref{fig:SPS8_SIGMA-Ulimit} shows a cross-section upper limit in terms of parameters $\Lambda$ and $\tau_{\PSneutralinoOne}$ of the region in parameter space where the ECAL sub-detector is most sensitive to late photons.  
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Mass and Lifetime Upper Limits}
Since the observed upper limit on $\sigma_{\mbox{\tiny{sig}}}\times BR_{\PSneutralinoOne}$ is expressed as a function of the neutralino mass and lifetime, it means we can equally set limits on  mass and lifetime of the neutralino.
\newline
For a given theoretical value of $\sigma_{\mbox{\tiny{sig}}}\times BR_{\PSneutralinoOne}$ as predicted by the SPS8 benchmack GMSB model, the exclusion limits~(lower and upper limit) in the lifetime of the neutralino are the $\tau_{\PSneutralinoOne}$ values for which the observed upper limit on $\sigma_{\mbox{\tiny{sig}}}\times BR_{\PSneutralinoOne}$ intersects with the theoretical value. For example, Figure \ref{fig:SPS8_Ctau_Ulimit} shows $\sigma_{\mbox{\tiny{sig}}}\times BR_{\PSneutralinoOne}$~(\pba) on the vertical axis as a function of $\tau_{\PSneutralinoOne}$~(\ns) on the horizontal axis for a neutralino of mass,  $m_{\PSneutralinoOne} = 255\GeVcc$. The points of intersection in $\tau_{\PSneutralinoOne}$ of the theoretical cross-section~(blue line) and the observed cross-section~(solid black line) are $\tau_{\PSneutralinoOne} = 3.2\ns$ and $\tau_{\PSneutralinoOne}=19.87\ns$ which we interpret as the lower limit and upper limit on the lifetime of the neutralino with as mass of 255\GeVcc, respectively. Thus, according to our search results neutralinos with mass of 255\GeVcc and lifetimes between 3.2\ns and 19.87\ns are excluded at 95\% CL. Outside this lifetime window, our method does not provide enough sensitivity for us to make  quantifiable statements.
\newline
In a similar manner, for a neutralino with a given life time the value of the mass where the theoretical cross-section intersects with the observed cross-section gives the upper limit of the mass of the neutralino. For example, in Figure \ref{fig:MASS-limits} the lifetime of the neutralino is, $\tau_{\PSneutralinoOne} = 6.7\ns$, and the theoretical cross-section~(blue line) intersects with the observed cross-section~(solid black line) at $m_{\PSneutralinoOne} = 275\GeVcc$. Thus, based on our search result neutralinos with lifetime, $\tau_{\PSneutralinoOne} = 6.7\ns$, and $m_{\PSneutralinoOne} \le  275\GeVcc$~($\Lambda < 195\TeV$) are excluded at 95\% CL.
\par   
A better way to see the excluded regions in lifetimes and mass of the neutralino is from a two dimensional exclusion plot defined by $(m_{\PSneutralinoOne},\tau_{\PSneutralinoOne})$ which are the shaded regions shown in Figure \ref{fig:SPS8_2D-Ulimit}. Our search results has excluded neutralinos with a mean lifetime ranging from $2.0$\ns to $45.0$\ns for a low mas range of $m_{\PSneutralinoOne} < 150\GeVcc$ while for larger masses up to $m_{\PSneutralinoOne} \leq 300\GeVcc$ we have excluded neutralinos with lifetime less than 10\ns all at 95\% CL. We compare our power of exclusion to those of other experiments~(CDF,ATLAS(7\TeV,8\TeV),CMS(7\TeV)). A common observation with all the experiments is that the excluded region in mean lifetime shrinks as the mass of the neutralino increases or equivalently the effective SUSY breaking scale increases. This is because the expected production cross-section for the lightest neutralino decreases with increasing effective SUSY breaking scale. 
...
\includegraphics[height=0.8\textwidth, width=0.85\textwidth]{THESISPLOTS/Final-Upper-Limit-Plot-180TeV_Neutralino_CrossSecTimesBR_Vs-Ctau.pdf} 
%}
\captionof{figure}{ 95\% $CL_{S}$ CL on the lightest neutralino production cross-section times branching ratio~($\sigma\times BR$)[\pba] against mean lifetime[\ns] for $\mathbf{\Lambda = 180}$\TeV in the SPS8 benchmark GMSB model.}
...
\begin{center}
%\mbox{
\includegraphics[height=0.8\textwidth, width=0.85\textwidth]{THESISPLOTS/Final-Upper-Limit-PLot-Ctau-2000mm-Neutralino_CrossSecVsMass.pdf}
%\includegraphics[height=0.65\textwidth, width=0.53\textwidth]{THESISPLOTS/Neutralino_CrosSecVsMass_Exclusion_limit_6000.pdf} }
\captionof{figure}{95\% $CL_{S}$ CL on the lightest neutralino production cross-section times branching ratio~($\sigma\times BR$) for different $\mathbf{\Lambda}$~(or mass of lightest neutralino) for $\tau = 6.7$\ns~($c\tau = 2000\mm$) in the SPS8 benchmark GMSB model.}
...
\includegraphics[height=0.85\textwidth, width=0.9\textwidth]{THESISPLOTS/Updated-2D-Limit-Plot.png}
\captionof{figure}{Exclusion limit in lightest neutralino mass~(or $\mathbf{\Lambda}$) against mean lifetime in SPS8 benchmark GMSB model. Limits from previous experiments also shown.}
...
\captionof{figure}{Cross-section upper limits at 95\% $CL_{S}$ CL for different mass and lifetime of the lightest neutralino using 8\TeV data corresponding to an integrated luminosity of 19.1\fbinv of the CMS experiment.}
