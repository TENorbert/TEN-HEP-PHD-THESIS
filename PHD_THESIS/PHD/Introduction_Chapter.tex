...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\par %\mbox{}\\
The search for Long-Lived Neutral Particles~(LLNP) produced in $pp$ collisions at $\sqrt{S} = 8$\TeV by the Large Hadron Collider~(LHC) on data recorded by the Compact Muon Solenoid~(CMS) detector is presented in this thesis.
\par
The Standard Model~(SM) describes the interactions of fundamental particles with unmatched precision and has been very successful in terms of all its predictions of the properties of particles agreeing with experiments. However, there are strong indications that the SM is incomplete. For example, the SM does not include gravity in its formulation and cannot describe Dark Matter~(DM) \cite{DM,SUSYDM,LSPDM}, which is about five times more abundant in the universe.
\par
There are many extensions to the SM where DM and other new phenomena are described. For example, many models Beyond the Standard Model~(BSM) predict that LLNPs, which are not described by the SM, can be produced at the LHC. These LLNPs can decay into stable, neutral particles which are good candidates for DM particles. The signature for the decay of a LLNP, is the presence of Missing Transverse Energy~(MET), which indicates the escape of the undetected candidate DM particle, and electromagnetic particles, like photons, which are detected late with respected to LHC $pp$ collision times, in the same event. The combination of a late photon and a large MET are the central idea in the search presented in this thesis.
\par
The LHC completed a successful first long operation in 2012. Data from $pp$ collisions during this period of operation~(LHC RUN 1) was recorded by the CMS detector. The CMS detector, including the Electromagnetic Calorimeter~(ECAL), was fully operational and recorded the arrival time and energy of electromagnetic particles produced from $pp$ collisions with excellent resolution. This resolution was maintained through out the entire LHC RUN 1 by continuous energy and timing performance monitoring and calibration. The data collected during this period is what we used in the search presented here.
\par 
Understanding the signature of a LLNP in CMS detector is provided using Monte Carlo simulated signal samples of the production and decay of LLNPs.
A brief introduction of the SM and Supersymmetry~(SUSY) and the description of GMSB models,\cite{GMSB}, with LLNPs is presented in Chapters 2. 
\newline 
In Chapter 3, the LHC and CMS particle detector are described with an emphasis on the sub-detectors used in this search for LLNPs. 
\newline 
A description of the photon timing measurements is discussed in Chapter 4, while the reconstruction  of events and their constituent particles is described in Chapter 5, where the definition of quantities like MET and jets is presented. 
\newline 
Chapter 6 describes the search method in detail including the data samples and triggers used, trigger efficiency, search observable, event selection requirements and background estimation techniques. The systematics arising from signal efficiency and acceptance in our signal event selection are also discussed in this chapter. In Chapter 7, we describe the statistical methods used to estimated the precision of the measurement.
\newline
Limits on production cross sections times branching ratio for different lifetimes and mass of a LLNP, within the context of our benchmark GMSB model is discussed in Chapter 8. The interpretation of our results in terms of the possible extensions to the SM by our analysis is also presented in this chapter and in Chapter 9 the conclusions are presented.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\label{intro_chapter}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
