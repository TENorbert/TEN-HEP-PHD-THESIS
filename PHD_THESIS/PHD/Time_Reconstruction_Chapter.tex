...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Timing Reconstruction and Resolution}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section*{ECAL Time Overview}}
The ECAL was designed to precisely measure the energy for electrons and photons produced in proton-proton collisions with a target barrel resolution of 0.5\% for photons with energies larger than 50\GeV. In addition to energy measurements, the combination of fast scintillation for \pb crystals, the electronic pulse shaping, and sampling rate of 40~MHz allow for excellent timing resolution to be obtained with the ECAL. The arrival time of a photon or electron is obtained from the pulse shape of the signal read from the front end electronics. The maximum height of the pulse represents the energy deposited and the time corresponding to this maximum height gives the particle's arrival time as measured by an individual crystal or channel. The pulse shapes are uniform and reproducible with a precision of 1\% among all the channels in the ECAL. The pulse is very precisely shaped and digitized at a rate of 40~MHz into 10 samples from
which the 6th 25~ns time sample is required to be the signal peak. 
To minimize channel-to-channel synchronization, a readout unit of $5\times5$ crystals are served by a common clock distribution line also synchronized with the LHC clock.
These $5\times5$ readout units represent a front end~(FE) readout unit. Similarly, because the
electromagnetic shower of a photon or electron spreads across several crystals, a cluster of about 25 crystals called the \textit{supercluster} after reconstruction represents an electromagnetic object.
Using the supercluster information, the arrival time of an electromagnetic object~(photon or electron) is defined either using the time of the crystal with the highest energy, called the \textit{seed crystal}, or an error weighted average of the individual times measured by each crystal belonging to the supercluster. A supercluster can also be made of many smaller clusters called \textit{basic clusters}, and the seed basic cluster time can also be used as the photon or electron's arrival time. We denote the seed time as $t_{seed}$, and, $t_{Ave}$, for the average time. In Equation \ref{eq:AVETIME}, we present the definition of $t_{Ave}$,
...
\end{equation}
where, $N$ is the number of crystals of the supercluster, $t_{reco,i}$  and $\sigma_{i}$ are the time and uncertainty on the reconstructed time, respectively, for each channel. 
...
This pre-amplifier has internal 40~ns pulse shaping and a floating-point Analog-to-Digital Converter~(ADC) with a 40~MHz sampling frequency by which the acquired pulse shape is digitized with 16 bit accuracy into 10 samples. The electronic noise  is about 40~MeV.
...
The digitized data of each channel is converted into an optical data stream which is transported off the detector through optical fibers to the upper-level off-detector where the formation of trigger tower energy sums, pipelining~(temporal storing of data until receipt of first level trigger decision) and transmission of triggered data to the data acquisition system is performed. The ECAL full electronics readout chain  is divided into an \textit{on-detector} and \textit{off-detector} electronics. Both electronic systems are connected by 100~m radiation hard optical fiber links for transporting the optical data stream. Figure \ref{fig:readout} is a schematic view of the complete ECAL signal readout electronics.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%\newline
...
The maximum amplitude value of the pulse, $\mathbf{A_{MAX}}$, and its corresponding time value, $\mathbf{T_{MAX}}$, is used to reconstruct the time recorded by that channel.  We define time reconstruction as the measurement of $\mathbf{T_{MAX}}$ using the 10 digitized samples pulse amplitude. The time reconstruction procedure is implemented using a timing algorithm which begins by first finding the maximum amplitude of the pulse. %Thus, ECAL timing reconstruction is using the 10 samples of the pulse amplitude to measure $\mathbf{T_{MAX}}$.
Finding the true $\mathbf{A_{MAX}}$ is not straight forward since  the amplitude of each channel depends on $\mathbf{A_{MAX}}$, the relative position of $\mathbf{T_{MAX}}$ between the time samples referred to as the "\textit{$T_{MAX}$ phase}" and the pulse shape itself.
As a result, an alternative representation of the pulse shape using the ratio variable defined as $R(T) = A(T)/ A(T + 25~\mbox{ns})$ is used as $R(T)$ is independent of $\mathbf{A_{MAX}}$.
This $ R(T)$ representation of the pulse shown in figure \ref{fig:amplVsTmax}(b), can easily be described using a polynomial function. %where the distribution of $T - \mathbf{T_{MAX}}$ as a function of $ R(T)$ is presented. This representation can easily be well described using a polynomial function.
...
\begin{center}
\centering
\mbox{\includegraphics[height=0.60\textwidth, width=0.90\textwidth]{THESISPLOTS/AmplitudeVsTMax.png}}
%%\includegraphics[height=0.45\textwidth, width=0.45\textwidth]{THESISPLOTS/TMaxPhaseVsRatio.png}}
%\includegraphics[scale=0.06]{THESISPLOTS/endcap_CMS.png}} 
\captionof{figure}{A measured ECAL pulse shape for a single channel(a). $T - T_{MAX}$ Vs $ R(T)$ showing the distribution of $T(R)$(b). Solid line is pulse shape from test beam while dots are the 10 discrete samples corresponding to signal from proton-proton collision.}
...
The polynomial paramterization of the pulse shape and its parameters were obtained during electron test beam experiments by fitting the pulse shape of the test beam data. These parameters obtained are subsequently use by the ECAL timing algorithm to reconstruct ECAL time during proton-proton collisions.
...
In order to quantify the performance of precise time measurement in ECAL, we study the total uncertainty also known as the \textit{time resolution}, $\sigma(t)$, is time measurement.% the time of electromagnetic objects by the crystals.
...
The full expression for timing resolution with all three contributions is given in equation \ref{eq:TimeRes},
...
Prior to installation at the CMS detector, about 25\% of the barrel and endcap crystals were exposed to electron beams with energy  between 15\GeV and 250\GeV. The time resolution was obtained from the distribution of the difference in time of two crystals sharing energy and belonging to the same electromagnetic shower. The advantage of this approach is that, the contribution from poor crystal-to-crystal synchronization is minimum. The average time definition was not used as it is easily affected by synchronization effects. %However, there are still crystal-to-crystal synchronization effects.
Other studies using the time difference between two basic clusters of separate electromagnetic shower was used and the results were similar. If we neglect the stochastic term its contribution to time resolution is negligible, equation \ref{eq:TimeRes} of the time resolution measured using two electromagnetic objects is reduced to equation \ref{eq:TIMERES}. We will be using equation \ref{eq:TIMERES} to expressed the ECAL time resolution for the purpose of convenience when comparing with the results from test beam.
...
In equation \ref{eq:TIMERES}, $A_{eff} = A_{1}A_{2}/\sqrt{A^{2}_{1} + A^{2}_{2}}$, while $t_{1,2}$ and $A_{1,2}$ are the times and amplitudes of the two objects. $\bar{C}$ is their residual constant term contribution.
We measure the time resolution using the standard deviation of a Gaussian fit to the time distribution from each slice of $A_{eff}/\sigma_{n}$ of the $A_{eff}/\sigma_{n}$ distribution. The resulting distribution of $\sigma(t_{1} - t_{2})$  of these standard deviations from the Gaussian fits of each slice against $A_{eff}/\sigma_{n}$ is used to extract the noise and residual constant terms.  The result is presented in Figure \ref{fig:FitTimeRes} with the noise factor $N = (35.1 \pm 0.2)$~ns and $\bar{C} = (20 \pm 4)$ ~ns from the distribution as the measured parameter values from test beam experiment. 
...
A time resolution obtained from test beam experiment is better than $100$~ps for energy $A_{eff}/\sigma_{n}$ greater than $400$~ADC counts. This demonstrates that for a perfectly calibrated ECAL crystals and energy deposits of $E > 20$~GeV in the barrel,  a time resolution better than 100~ps is possible. %we can obtain a resolution better than $100$~ps. 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{ECAL Time Performance from Collision}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
It is imperative to archive very precise time measurements with ECAL during proton-proton collision.
As precise time measurement can be used in the rejection of unwanted electromagnetic objects with broad time distribution and can  equally allow for the use of ECAL time measurements to search for electromagnetic objects produced from the decay of trapped or long-lived particles.
Some of these unwanted objects are cosmic rays, beam halo muons, electronic noise and out-of-time proton-proton interactions.
In order to realized very precise time measurements, worsening of the time resolution from effects like crystal time variations over a period of time, timing bias with energy, crystal-to-crystal synchronization and possibly crystal radiation which can potentially limit precise time measurements will have to be studied and their contributions removed.
\subsection{Time Calibrations}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Variations in the crystal  reconstructed time due to non-uniform response to electromagnetic showers by every ECAL crystals or  time shifts caused by interventions during detector repairs can limit precise time measurements. To avoid contributions from these effects towards imprecise time measurements, we time align all the 75,848 \pb crystals in ECAL in a process called \textit{time calibration}. 
Time calibration ensures that there is a uniform response by every crystal to photons produced from proton-proton collisions, at interaction point, and traveling along a straight path with speeds near the speed of light~( $\beta \approx 1$) to the surface of any crystal. It equally guarantees the synchronization of all the component objects of a given event and assigns each event to the correct LHC proton-proton collision or bunch crossing. 
\newline
The presence of the "$T_{MAX}$ Phase", the difference in pulse shape between each crystal, variation in time of flight by a few nanosecond~(ns) and the different intrinsic delays among channels allow for timing calibration at two separate levels. At the level of the front end electronics ~(FE) consisting of $5\times5$ crystals, one is capable of performing an initial internal timing synchronization by adjusting in steps of 1.04~ns among these block of crystals. Determining the values of adjustments to be made and immediately~(online) performing this adjustments is called \textit{Hardware Synchronization}. Offline, during event reconstruction, we can find and assign timing constants such that the measured crystal time for photon objects is on average zero for true collision events arriving to the crystals directly on straight path from the proton-proton interaction point.
The time adjustments constants are developed separately for the barrel and endcaps.
There are significant benefits to archiving a uniform time alignment by all the crystals as with a well time calibrated detector, one can easily distinguished between photons or electrons arriving at an ECAL crystal with significantly large arrival time from normal in-time photons produced in collisions. Such photons with large time could be produced from slow moving particles~($\beta << 1$) or could be photons traveling along a path which significantly deviates from the obvious straight path of photons coming directly from proton-proton interaction point. Large arrival time photons or electrons could also be produced in the decay of temporarily stopped particles inside the CMS detector. 

\subsubsection{Offline Time Calibration}
The purpose of the offline calibration is to provide time adjustments~(calibration) constants for each channel~(crystal) during event reconstruction. These constants are derived from data recorded during  proton-proton collisions and are used to adjust for any global time shift in time measurements that might have occurred. This global phase shift is caused by time shifts in the CMS clock relative to the LHC clock and equally through de-synchronizations introduced during hardware interventions during detector repairs. The global time shifts caused by CMS clock accumulated time drifts can be about 1~ns while that caused by hardware intervention can be $3$ to $5$~ns.
The calibration constants for each crystal is the reverse sign of the average time of all the reconstructed events of the crystal.
...
A particular set of time constants produced in a given instance has an \textit{interval of validity}~(IOV)
...
These events are required to undergo a rigorous event selection process in order to minimize contamination from events not produced
in proton-proton collisions like cosmic muons, beam halos and electronic noise. The produced constants are validated in a closure test procedure to be used for further event reconstruction.
The maps in Figure \ref{fig:TimeCalib}, show two dimensional distribution maps of the average time~(time calibration constants) for each of the 61,200 crystals in EB and 14648 crystals in EE. The figure shows each crystal average time before~(\textit{top 3 plots}) and after~(\textit{bottom 3 plots}) calibration. More information about crystal time calibration for the entire LHC Run 1 can be found in \cite{ECALCAL}.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{center}
\centering
...
A time offset  is introduced during repairing the hardware of ECAL front end electronics. 
To minimize the impact of these time offsets and reduce additional rounds of necessary event re-reconstruction, it is 
efficient and effective to adjust the time offsets or time latency directly online during CMS data recording.
This adjustment can be performed using data from proton-proton collision or from laser. 
...
The approach of adjusting hardware latency or timing offsets online during data recording by CMS ECAL with stable proton-proton collision beams is to temporarily stop the entire data recording process, use the fraction of data collected thus far, and adjust the hardware latency. Data recording is resumed once the time latency is adjusted. The  Data Quality Monitoring~(DQM) and
data certification expert teams are heavily in used during this process. The act of stopping the CMS data recording process during LHC stable proton beams evidently leads to data lost causing the large difference between luminosity recorded by CMS compared to luminosity delivered by the LHC.
As a result, although online or hardware time adjustment using this collision data approach is efficient, however, the large CMS data recording downtime and lost of luminosity makes it unreliable especially during frequent machine interventions.
To remedy the situation, laser based time latency adjustment approach is encouraged.
...
The ECAL laser system comprise of two lasers, a 440~nm wavelength~(close to peak emission for \pb crystals) laser for monitoring crystal transparency lose and a 796~nm wavelength laser for monitoring readout electronics chain from photodetectors to the electronics~(APDs to ADCs). Both lasers have a jitter of less than 4~ns every 24 hours run and as a result, the time from the lasers is averaged over 600 event pulses. The time for each crystal from the laser is expected to be the same as the time from collision data and is represented as $\mathbf{T^{APD}_{MAX}}$. The laser system is also equipped with a fast acquisition card called MATACQ. The time for each channel recorded using the Matacq is also averaged over 600 event pulse and denoted  $\mathbf{T_{MATACQ}}$.
The difference, $ \mathbf{T^{APD}_{MAX}} - \mathbf{T_{MATACQ}}$, between these two times, averaged over the  25 crystals of a Clock and Control Unit ~(CCU) is used as the time for each CCU, $ t_{CCU} = \left\langle \mathbf{T^{APD}_{MAX} - T_{MATACQ}} \right\rangle $. 
...
Using laser data, we are able to measure the time shift of each CCU to within 0.2~(0.5)~ ns EB~(EE) in precision.
Compared to using collision data approach, there are no downtime as we can adjust for any latency in hardware time prior to stable proton beams.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
An additional source which contributes to poor time measurement is energy related time bias effects.
These originate from using the ratio approach to reconstructing the time. The ratio method is assumed to perform efficiently for all ADC counts. However, during data recording in LHC Run 1, it was observed that for high ADC counts which can only be produced during proton-proton collisions, an inherent bias in the time is introduced by the multi-gain pre-amplifier electronics for electromagnetic particles with energy above certain gain transition points. 
