...
An electromagnetic particle which arrived at ECAL from proton-proton~($pp$) collisions is observed in the front end electronics as a pulse. Using the pulse's maximum height, the particle's arrival time is  measured.
The height of the pulse represents the energy while the time corresponding to this maximum height represents the particle's arrival time.
The analog pulse shapes are uniform and reproducible with a precision of 1\% among all the channels in the ECAL.
The pulse is very precisely shaped and digitized at a rate of 40~MHz. From this digitized signal pulse, 10 samples corresponding to the entire pulse are readout and the 6th 25~ns time sample is required to be the signal peak. 
To minimize channel to channel synchronization, a readout unit of $5\times5$ crystals are served by a common clock distribution line.
These $5\times5$ readout units represents a front end~(FE) readout unit. It is also worth noting that a single photon or electron object
is at the level of the ECAL, a \textit{superclusters} consisting of at most $25$ crystals.
\paragraph*{}
Therefore, the arrival time of a photon or electron is defined either using the time of the crystal with the highest energy which we
call the \textit{seed crystal} or an error weighted average or mean time of the individual time measured by crystals belonging to the photon supercluster. A supercluster can also be made of many smaller clusters called \textit{basic clusters}). We denote the seed time as $t_{seed}$ and $t_{Ave}$ for the average time. Equation \ref{eq:AVETIME} is the expression for the average time.
