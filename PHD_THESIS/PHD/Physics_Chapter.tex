...
\newline
Fermions acquire their mass through their interaction with the Higgs field and the fermion's mass, $m_{f}$, is proportional to the strength of its interaction (\text{Yukawa} coupling $\lambda_{f}$) with the Higgs field. The relationship between the masses of the fermions and the \PW,\PZ masses is given in Equation \ref{eq:FMASS}.
\newline
...
\captionof{figure}{SUSY mass spectra for benchmark SPS8 model: $\mathbf{\Lambda} = 100$\TeV~(left) and $\mathbf{\Lambda} = 180$\TeV~(right) with $C_{grav} = 93.5$. SUSY particle mass increases with $\mathbf{\Lambda}$ }
...
Thus, by changing the value of $C_{grav}$, we change the lifetime of \PSneutralinoOne, similar to changing the value of $\mathbf{\Lambda}$, we get \PSneutralinoOne with different masses.
\par 
...
\subsection{Lightest Neutralino's Lifetime}
