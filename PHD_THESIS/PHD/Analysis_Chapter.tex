...
\includegraphics[scale=0.2]{THESISPLOTS/CMS_Slice.png}
\includegraphics[scale=0.2]{THESISPLOTS/CMS_Slice.png}
\captionof{figure}{ECAL time Vs $\eta$ and ECAL time Vs $\phi$~(left) for photons from SinglePhoton dataset~(left) similar plots from the DoubleElectron dataset~(right). Photons at $\phi = 0, \pm \pi$ which are mostly halo photons not observed in the $Z$ boson candidate sample.}
\label{fig:Elec}
\end{center}
...
\includegraphics[scale=0.2]{THESISPLOTS/CMS_Slice.png}
\includegraphics[scale=0.2]{THESISPLOTS/CMS_Slice.png}
\captionof{figure}{Di-electron candidate mass distribution and the time of both electrons for the signal $71 < |m_{\ell_{1}, \ell_{2}} < 100$~$GeV/c^{2}$ $Z$ boson sample(left) and similar distributions from the Control~(outside signal region) sample~(right). Candidates events from the DoubleElectron dataset.}
\label{fig:Zmass}
\end{center}
