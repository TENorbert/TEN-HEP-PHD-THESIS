...
Each LCH proton bunch is placed inside an RF buckets during beam filling. % with where each RF bucket has an energy against time profile as can be seen in figure below.
The filling scheme is such that not all RF buckets have proton bunches. 
Those empty buckets or beam gaps are necessary to make room for beam dump and cleaning.%the rise/fall times at the SPS/LHC injection and ejection and to abort kickers magnets during
...
(Right) Timing of Clusters in the CMS endcap calorimeters for fill 1089:Left: EEP detector(left side of IP $z> 0$) Right: EEM detector( right side of IP, $z<0$).  Plots from ATLAS Ref.\cite{ATLAS-GHOST} and CMS, Ref.\cite{CMS-GHOST}}
...
We provide in table \ref{tab:tableLHC}, a summary, showing LHC design conditions and operation conditions during the LHC RUN 1. 
\begin{center}
%\begin{sidewaystable}
 \begin{table}[H]
  \begin{turn}{90}
...
\centering
 ...
 Tracker   & Momentum[GeV/c]  & $\sigma_{T}/p_{T} \approx 1.5\times 10^{-4}p_{T} + 0.005$ & \mbox{Silicon Pixels and Strips} \\ 
  ...
  \hline
  \end{tabular}
  \captionof{table}{CMS detector material, Ref.\cite{CMSTDR}, and resolution(Time resolution: $N\approx 35$~ns, $\bar{C}\approx 0.070$~ns Ref. \cite{ECALTIME2014}) }
 \label{tab:tableCMS}
 \end{center}
%\subsection{Tracker}
...
The hadronic calorimeter unlike the ECAL, is made of alternating plates of brass and plastic scintillator. The brass plate is used for stopping energetic hadrons like protons, kaons and pions while the plastic scintilator is for measuring their
...
The  hadronic barrel(\text{HB}) and hadronic endcap(\text{HE}) of the HCAL cover a combine region in pseudo-rapidity of $\vert \eta \vert < 3$. 
...
L1 triggers are hardware designed electronics system while the HLT is a farm of more than 1000 standard computers running the HLT event selection algorithm. %Thus event selection begins at the L1 and HLT triggers.
\newline
